...
- (void)commonInit {
    ...
    self.font = [UIFont fontWithName:@"Helvetica-Bold" size:15.000];
...
-(void)addLineGreyScrollView
{
    CALayer *lineGrey = [CALayer layer];
    lineGrey.frame = CGRectMake(0, self.scrollView.frame.size.height-2, self.scrollView.contentSize.width, 2);
    UIColor *grey =  [UIColor colorWithRed:137/255.0f green:137/255.0f blue:137/255.0f alpha:1.0f];
    lineGrey.backgroundColor = grey.CGColor;
    [self.scrollView.layer addSublayer:lineGrey];
}
...
- (void)drawRect:(CGRect)rect {
    ...
    if (self.type == HMSegmentedControlTypeText) {
        [self.sectionTitles enumerateObjectsUsingBlock:^(id titleString, NSUInteger idx, BOOL *stop) {
            ...
            + ((self.selectionIndicatorLocation == HMSegmentedControlSelectionIndicatorLocationUp) ? self.selectionIndicatorHeight : 0)/2
            ;
